...
export default function Home() {
  ...
  const checkout = trpc.payments.createCheckout.useMutation({
    onSuccess: (result) => {
      if ("alreadyOwned" in result && result.alreadyOwned) {
        window.alert("This account already has the JEDI Hub Premium Upgrade.");
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#050912] text-[#edf9f7] selection:bg-[#32f2c0] selection:text-[#04110e]">
      ...
      <section id="billing" className="relative z-10 border-t border-white/10 bg-[#08121c]/90 px-5 py-20 sm:px-8 sm:py-24"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_.8fr]"><div><p className="text-xs font-bold uppercase tracking-[.22em] text-[#32f2c0]">Access / 03</p><h2 className="mt-4 text-4xl font-semibold tracking-[-.05em] sm:text-5xl">Free now. Ready for scale.</h2><p className="mt-5 max-w-xl text-[#91aaa8]">Free-track downloads remain available to everyone. Optional paid upgrades unlock enhanced deployment controls, priority release channels, and advanced marketplace support through secure Stripe-hosted Checkout.</p><div className="mt-8 flex flex-wrap gap-3"><button onClick={() => setShowSignup(true)} className="inline-flex items-center gap-2 rounded-xl bg-[#32f2c0] px-5 py-3 text-sm font-bold text-[#04110e]"><UserPlus size={17} />Register free</button><button onClick={() => setShowCheckout(true)} className="inline-flex items-center gap-2 rounded-xl border border-white/15 px-5 py-3 text-sm font-semibold text-[#c8dbd8] hover:border-[#32f2c0]/40"><CircleDollarSign size={17} />Upgrade options</button></div></div><div className="rounded-2xl border border-[#a879ff]/20 bg-[#a879ff]/[.05] p-7"><div className="flex items-center gap-3"><Zap className="text-[#c8a7ff]" size={20} /><p className="font-semibold">Connector gate</p></div><p className="mt-4 text-sm leading-6 text-[#afc0be]">Free-track downloads are live and never require payment. Optional upgrades are available in Stripe test mode for authenticated users; fulfillment is granted only after a verified webhook.</p><div className="mt-6 flex items-center gap-2 text-xs text-[#c8a7ff]"><span className="size-2 rounded-full bg-[#c8a7ff]" />OPTIONAL UPGRADES READY</div></div></div></section>
      ...
      {showCheckout && <div className="fixed inset-0 z-50 grid place-items-center bg-[#02050a]/80 p-5 backdrop-blur-md"><div className="w-full max-w-md rounded-2xl border border-[#a879ff]/30 bg-[#0a1721] p-7 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#c8a7ff]">Access console</p><h2 className="mt-3 text-2xl font-semibold">Optional upgrades</h2></div><button onClick={() => setShowCheckout(false)} aria-label="Close access dialog"><X size={20} className="text-[#8da5a2]" /></button></div><p className="mt-5 text-sm leading-6 text-[#9bb4b1]">Free-track downloads work without an account or payment. Sign in to open the secure Stripe Checkout for the optional JEDI Hub Premium Upgrade; the test-mode price is $9.99 and no card data is stored here.</p><div className="mt-7 space-y-3"><button onClick={launchCheckout} disabled={authLoading || checkout.isPending} className="w-full rounded-lg bg-[#32f2c0] px-4 py-3 text-sm font-bold text-[#04110e] disabled:cursor-wait disabled:opacity-60">{checkout.isPending ? "Opening secure checkout…" : "Continue to optional upgrade"}</button><button onClick={() => setShowCheckout(false)} className="w-full rounded-lg border border-white/15 px-4 py-3 text-sm font-semibold text-[#c8dbd8]">Return to hub</button>{checkout.error && <p className="text-xs leading-5 text-[#f3a7c4]">{checkout.error.message}</p>}</div></div></div>}
    </main>
  );
}
