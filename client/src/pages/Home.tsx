...
import { ArrowDownToLine, Check, ChevronDown, Copy, Github, Menu, ShieldCheck, Smartphone, X } from "lucide-react";
...
export default function Home() {
  ...
  const trackDownload = () => {
    ...
    analytics?.track("apk_download", { version: "1.0.32", platform: "android" });
  };
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#071016] text-[#f1f7f3] selection:bg-[#1bda91] selection:text-[#06100c]">
      ...
      <header className="relative z-10 border-b border-white/10 bg-[#071016]/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
          ...
          <nav className="hidden items-center gap-8 text-sm text-[#9fb3a8] md:flex">
            ...
            <a className="transition hover:text-white" href="#share">Share</a>
      ...
      <section id="top" className="relative z-0 mx-auto grid max-w-7xl items-center gap-14 px-5 pb-24 pt-20 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:gap-20 lg:pb-32 lg:pt-28">
        <div>
          ...
          <div className="mt-10 flex flex-col gap-3 sm:flex-row sm:items-center">
            <a href={APK_URL} download onClick={trackDownload} className="group inline-flex items-center justify-center gap-3 rounded-xl bg-[#1bda91] px-6 py-4 text-sm font-bold text-[#06100c] shadow-[0_12px_40px_rgba(27,218,145,.2)] transition duration-200 hover:-translate-y-0.5 hover:bg-[#69efb9] active:scale-[.98]"><ArrowDownToLine size={19} />Download APK<span className="text-[#0b6547]">↗</span></a>
      ...
      <section id="install" className="relative border-y border-white/10 bg-[#0a1519] py-20 sm:py-24"><div className="mx-auto max-w-7xl px-5 sm:px-8"><div className="max-w-2xl"><p className="text-xs font-semibold uppercase tracking-[.2em] text-[#1bda91]">Three steps</p><h2 className="mt-4 text-4xl font-semibold tracking-[-.04em] sm:text-5xl">From link to launch.</h2><p className="mt-5 text-[#9fb3a8]">Android may ask for one permission the first time you install an APK outside the Play Store. That is expected.</p></div><div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 md:grid-cols-3">{steps.map(([number, title, copy]) => <article key={number} className="bg-[#0a1519] p-7 transition hover:bg-[#10201f]"><span className="font-mono text-sm text-[#1bda91]">{number}</span><h3 className="mt-12 text-xl font-semibold">{title}</h3><p className="mt-3 text-sm leading-6 text-[#8fa39a]">{copy}</p></article>)}</div><div id="compatibility" className="mt-10 grid gap-4 rounded-2xl border border-white/10 bg-white/[.03] p-6 sm:grid-cols-3"><div><p className="text-xs uppercase tracking-[.18em] text-[#789087]">Platform</p><p className="mt-2 font-semibold text-[#dceae2]">Android phones & tablets</p></div><div><p className="text-xs uppercase tracking-[.18em] text-[#789087]">Recommended</p><p className="mt-2 font-semibold text-[#dceae2]">Android 8.0 or newer</p></div><div><p className="text-xs uppercase tracking-[.18em] text-[#789087]">Space needed</p><p className="mt-2 font-semibold text-[#dceae2]">At least 100 MB free</p></div></div></div></section>

      <section id="release" className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-24"><div className="grid gap-12 lg:grid-cols-[1fr_.8fr]"><div><p className="text-xs font-semibold uppercase tracking-[.2em] text-[#1bda91]">Release notes</p><h2 className="mt-4 text-4xl font-semibold tracking-[-.04em] sm:text-5xl">A clean, direct build.</h2><p className="mt-5 max-w-xl text-[#9fb3a8]">This free-track release is distributed through GitHub Releases so the package stays transparent, stable, and easy to verify.</p></div><div className="rounded-2xl border border-white/10 bg-white/[.03] p-6"><div className="flex items-center justify-between border-b border-white/10 pb-4"><span className="text-sm font-semibold">v1.0.32</span><span className="rounded-full bg-[#1bda91]/10 px-3 py-1 text-xs text-[#62eab2]">Latest</span></div><dl className="mt-5 space-y-4 text-sm"><div className="flex justify-between gap-4"><dt className="text-[#789087]">Format</dt><dd className="font-mono text-[#dceae2]">Android APK · {APK_SIZE}</dd></div><div className="flex justify-between gap-4"><dt className="text-[#789087]">Distribution</dt><dd className="text-[#dceae2]">Free track</dd></div><div className="flex justify-between gap-4"><dt className="text-[#789087]">Source</dt><dd><a className="inline-flex items-center gap-1 text-[#1bda91] hover:underline" href="https://github.com/MasterJediTek/jedi-medivac-download" target="_blank" rel="noreferrer">GitHub <Github size={14} /></a></dd></div></dl><div className="mt-6 border-t border-white/10 pt-5"><div className="flex items-center justify-between gap-3"><p className="text-xs uppercase tracking-[.16em] text-[#789087]">SHA-256 checksum</p><button onClick={copyChecksum} className="inline-flex items-center gap-1.5 text-xs text-[#1bda91] transition hover:text-[#69efb9]" aria-label="Copy SHA-256 checksum">{copied ? <Check size={14} /> : <Copy size={14} />}{copied ? "Copied" : "Copy"}</button></div><p className="mt-2 break-all font-mono text-xs leading-5 text-[#c8d7cf]">{APK_SHA256}</p><p className="mt-2 text-xs leading-5 text-[#789087]">Compare this value with the file you downloaded to confirm the APK was not altered.</p></div></div></div></section>
      <section id="share" className="border-t border-white/10 bg-[#0a1519] px-5 py-20 sm:px-8 sm:py-24"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[.9fr_1.1fr]"><div className="rounded-2xl border border-white/10 bg-[#071016] p-7"><p className="text-xs font-semibold uppercase tracking-[.2em] text-[#1bda91]">Share the release</p><h2 className="mt-4 text-3xl font-semibold tracking-[-.04em]">Scan on Android.</h2><p className="mt-4 max-w-sm text-sm leading-6 text-[#9fb3a8]">Open your phone camera and scan this code to return to the official download page.</p><div className="mt-7 flex items-center gap-5"><a href="https://medivacapk-ehyatfv7.manus.space" target="_blank" rel="noreferrer" className="rounded-xl bg-white p-2 transition hover:scale-[1.02]" aria-label="Open Medivac One download page"><img className="size-32" src="https://api.qrserver.com/v1/create-qr-code/?size=180x180&margin=8&data=https%3A%2F%2Fmedivacapk-ehyatfv7.manus.space" alt="QR code for the Medivac One download page" /></a><div className="text-xs leading-5 text-[#789087]"><p className="font-mono text-[#dceae2]">medivacapk-ehyatfv7.manus.space</p><p className="mt-2">Free-track distribution<br />No account required</p></div></div></div><div className="rounded-2xl border border-white/10 bg-[#071016] p-7"><p className="text-xs font-semibold uppercase tracking-[.2em] text-[#1bda91]">Changelog</p><div className="mt-5 space-y-6"><div className="border-l border-[#1bda91]/50 pl-5"><div className="flex flex-wrap items-center gap-3"><h3 className="font-semibold">v1.0.32</h3><span className="rounded-full bg-[#1bda91]/10 px-2.5 py-1 text-[10px] uppercase tracking-[.15em] text-[#62eab2]">Current</span></div><p className="mt-2 text-sm leading-6 text-[#9fb3a8]">Free-track Android release with direct GitHub distribution and verified SHA-256 integrity.</p></div><div className="border-l border-white/15 pl-5"><h3 className="font-semibold text-[#dceae2]">Download page update</h3><p className="mt-2 text-sm leading-6 text-[#9fb3a8]">Added compatibility guidance, checksum verification, privacy-conscious download analytics, and mobile QR access.</p></div></div><a href="https://github.com/MasterJediTek/jedi-medivac-download/releases" target="_blank" rel="noreferrer" className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-[#1bda91] hover:underline">View all releases <Github size={15} /></a></div></div></section>
